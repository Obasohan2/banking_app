...
def deposit(account, amount):
    row, name, balance, _ = find_account(account)
    if row is None:
        print("Account not found.\n")
        return
...
def withdraw(account, amount):
    ...
    print(f"Withdrawn £{amount:.2f} from {name}")
    print(f"New balance   : £{new_balance:.2f}")
    print(f"Updated       : {now}\n")


def display_balance(account):
    _, name, balance, last_upd = find_account(account)
    if balance is None:
        print("Account not found.\n")
    else:
        print(f"Account holder: {name}")
        print(f"Account number: {format_account_number(account)}")
